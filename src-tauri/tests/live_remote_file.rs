...
use durable_terminal_lib::remote_file::read_remote_file;
...
fn live_fetch_text_and_truncation_and_binary() {
    ...
    // text file, full read
    let f = read_remote_file(&host, "/tmp/dt_view_test.txt", 1_000_000, &[]).expect("read text");
    ...
    let t = read_remote_file(&host, "/tmp/dt_view_test.txt", 5, &[]).expect("read capped");
    ...
    // markdown file
    let m = read_remote_file(&host, "/tmp/dt_view_test.md", 1_000_000, &[]).expect("read md");
    assert!(String::from_utf8_lossy(&m.data).contains("# Title"));

    // binary file: bytes survive base64 transport intact (no corruption)
    let b = read_remote_file(&host, "/tmp/dt_view_big.bin", 1_000_000, &[]).expect("read bin");
    assert_eq!(b.data.len(), 200_000, "full binary size preserved");
    assert!(!b.truncated);

    // a path that isn't a file -> error, not a panic
    assert!(read_remote_file(&host, "/tmp", 1000, &[]).is_err(), "directory rejected");
    assert!(read_remote_file(&host, "/no/such/path/xyz", 1000, &[]).is_err(), "missing rejected");
